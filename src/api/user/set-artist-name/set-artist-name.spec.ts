...
describe('/api/user/set-artist-name', () => {
  ...
  describe('authorized access', () => {
    it('should reject guest access', async () => {
      const { error } = await api.PATCH(`/api/user/set-artist-name`, {
        body: {
          name: 'Custom Artist',
          title: 'Custom title',
          year: 2026,
  ...
  describe('errors', () => {
    it('should reject invalid artist id', async () => {
      const { error } = await userApi.setArtistName(-1, {
        name: 'Custom Artist',
      });
    ...
    it('should reject invalid name length', async () => {
      const { error } = await userApi.setArtistName(artistId, {
        name: 'a'.repeat(1001),
      });
  ...
  describe('success', () => {
    it('should create custom data for the file', async () => {
      ...
      const { error, data } = await userApi.setArtistName(artistId, {
        name: 'Custom Artist',
      });
